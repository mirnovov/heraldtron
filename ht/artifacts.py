import json, random, os, functools, re
...
async def rijksmuseum():
	...
	result = random.choice( list(collection["artObjects"]) )
...
async def victoria_and_albert():
	...
	result = random.choice(list(collection["records"]))
...
async def europeana():
	...
	result = random.choice(list(collection["items"]))
	return (
		result["guid"], 
		result["title"][0], 
		"",
...
async def digital_nz():
	...
	results = list(collection["search"]["results"])
